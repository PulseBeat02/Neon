...
public interface Locale extends LocaleParent {
  ...
  NullComponent<Sender> INVALID_HOMEPAGE_URL = error("neon.command.invalid-homepage-url");
  UniComponent<Sender, String> INVALID_DITHER_ALGORITHM =
      error("neon.command.browser.invalid-dither", null);
  ...
  BiComponent<Sender, Integer, Integer> SET_RESOLUTION =
      info("neon.command.browser.resolution", null, null);
  BiComponent<Sender, Integer, Integer> SET_BLOCK_DIMENSION =
      info("neon.command.browser.block-dimension", null, null);
}
