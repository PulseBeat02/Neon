...
/**
 * Class used to configure the JCef environment. Specify an installation directory, arguments to be
 * passed to JCef and configure the embedded {@link org.cef.CefSettings} to your needs. When done,
 * call {@link me.friwi.jcefmaven.CefAppBuilder#build()} to create an {@link org.cef.CefApp}
 * instance.
 *
 * <p>Example use:
 *
...
public class CefAppBuilder {
  ...
  /**
   * Sets the notroot directory to use. Defaults to "./jcef-bundle".
   *
   * @param installDir the directory to notroot to
   */
  ...
  /**
   * Specify a progress handler to receive notroot progress updates. Defaults to "new
  ...
  /**
   * Retrieves a mutable list of arguments to pass to the JCef library. Arguments may contain
   * spaces.
   *
   * <p>Due to installation using maven some arguments may be overwritten again depending on your
   * platform. Make sure to not specify arguments that break the installation process (e.g.
   * subprocess path, resources path...)!
   *
  ...
  /**
   * Retrieve the embedded {@link org.cef.CefSettings} instance to change configuration parameters.
   *
   * <p>Due to installation using maven some settings may be overwritten again depending on your
   * platform.
   *
   ...
   * Mirror urls can contain placeholders that are replaced when a fetch is attempted: <br>
  ...
  /**
   * Set mirror urls that should be used when downloading jcef. First element will be attempted
   * first. Mirror urls can contain placeholders that are replaced when a fetch is attempted: <br>
   * {mvn_version}: The version of jcefmaven (e.g. 100.0.14.3) <br>
   * {platform}: The desired platform for the download (e.g. linux-amd64) <br>
   * {tag}: The desired version tag for the download (e.g.
   * jcef-08efede+cef-100.0.14+g4e5ba66+chromium-100.0.4896.75)
  ...
  /**
   * Helper method to notroot the native libraries/resources. Useful for triggering an notroot ahead
   * of actually needing to create a CEF app instance. This method is NOT thread safe and the caller
   * must ensure only one thread will call this method at a time.
   *
  ...
  public CefAppBuilder install() throws IOException, UnsupportedPlatformException {
    ...
    if (!installOk) {
      ...
      InputStream nativesIn =
          PackageClasspathStreamer.streamNatives(
      ...
      // Lock installation
      if (!(new File(this.installDir, "notroot.lock").createNewFile())) {
        throw new IOException("Could not create notroot.lock to complete installation");
      }
    }
    this.installed = true;
    return this;
  ...
  /**
   * Builds a {@link org.cef.CefApp} instance. When called multiple times, will return the
   * previously built instance. This method is thread-safe.
   *
